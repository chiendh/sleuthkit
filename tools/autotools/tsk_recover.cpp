...
#include "./win32/tsk_recover/Disk.h"
...
void ListPhysicalDrives() {
    ...
    TCHAR szPhysicalDrive[MAX_PATH] = TEXT("");

    while (true) {
        // Tạo chuỗi tên ổ đĩa
        _stprintf_s(szPhysicalDrive, MAX_PATH, TEXT("\\\\.\\PhysicalDrive%u"), driveIndex);

        // Mở ổ đĩa vật lý
        HANDLE hDrive = CreateFile(szPhysicalDrive, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);

        if (hDrive == INVALID_HANDLE_VALUE) {
            // Nếu không thể mở ổ đĩa, giả sử chúng ta đã liệt kê qua tất cả các ổ đĩa
            break;
        }

        // In tên ổ đĩa
        std::wcout << L"Found: " << szPhysicalDrive << std::endl;

        // Đóng handle ổ đĩa
        CloseHandle(hDrive);

        // Chuyển sang ổ đĩa tiếp theo
        driveIndex++;
    }

    if (driveIndex == 0) {
        std::wcout << L"No physical drives found." << std::endl;
    }
}

void PrintDriveInfo(DWORD driveIndex) {
    TCHAR szDrivePath[MAX_PATH];
    HANDLE hDrive;
    DWORD dwBytesReturned = 0;
    DISK_GEOMETRY dg;

    _stprintf_s(szDrivePath, MAX_PATH, TEXT("\\\\.\\PhysicalDrive%u"), driveIndex);
    hDrive = CreateFile(szDrivePath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);

    if (hDrive == INVALID_HANDLE_VALUE) {
        std::wcout << L"Unable to access " << szDrivePath << std::endl;
        return;
    }

    std::wcout << L"\nDrive: " << szDrivePath << std::endl;

    // Get the disk geometry
    if (DeviceIoControl(hDrive, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &dg, sizeof(dg), &dwBytesReturned, NULL)) {
        LONGLONG DiskSize = dg.Cylinders.QuadPart * (ULONG)dg.TracksPerCylinder * (ULONG)dg.SectorsPerTrack * (ULONG)dg.BytesPerSector;
        std::wcout << L"Disk Size: " << DiskSize << L" bytes" << std::endl;
        std::wcout << L"Sectors per track: " << dg.SectorsPerTrack << std::endl;
        std::wcout << L"Tracks per cylinder: " << dg.TracksPerCylinder << std::endl;
        std::wcout << L"Bytes per sector: " << dg.BytesPerSector << std::endl;
    }
    else {
        std::wcout << L"Failed to get disk geometry." << std::endl;
    }

    CloseHandle(hDrive);
}

bool GetDriveGeometry(LPWSTR wszPath, DISK_GEOMETRY& dg) {
    HANDLE hDevice = CreateFileW(wszPath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    if (hDevice == INVALID_HANDLE_VALUE) {
        return false;
    }

    DWORD bytesReturned;
    bool result = DeviceIoControl(hDevice, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &dg, sizeof(dg), &bytesReturned, (LPOVERLAPPED)NULL);
...
int main() {
    ...
    ListPhysicalDrives();
    for (DWORD i = 0;; i++) {
        WCHAR wszDriveName[256];
        wsprintfW(wszDriveName, L"\\\\.\\PhysicalDrive%d", i);

        DISK_GEOMETRY dg = { 0 };
        if (GetDriveGeometry(wszDriveName, dg)) {
            std::wcout << L"\nDrive: " << wszDriveName << std::endl;

            // Tính toán kích thước ổ đĩa
            LONGLONG DiskSize = dg.Cylinders.QuadPart * (ULONG)dg.TracksPerCylinder * (ULONG)dg.SectorsPerTrack * (ULONG)dg.BytesPerSector;

            // Đổi kích thước ổ đĩa sang MB
            double DiskSizeMB = static_cast<double>(DiskSize) / (1024 * 1024);
            double DiskSizeGB = DiskSizeMB / 1024;

            std::wcout << L"Disk Size: " << DiskSizeMB << L" MB " << DiskSizeGB << L" GB" << std::endl;
            std::wcout << L"Sectors per track: " << dg.SectorsPerTrack << std::endl;
            std::wcout << L"Tracks per cylinder: " << dg.TracksPerCylinder << std::endl;
            std::wcout << L"Bytes per sector: " << dg.BytesPerSector << std::endl;
        }
            ...
            if (GetLastError() == ERROR_FILE_NOT_FOUND) {
                break; // Không tìm thấy ổ đĩa vật lý tiếp theo, dừng vòng lặp
            }
        }
    }
